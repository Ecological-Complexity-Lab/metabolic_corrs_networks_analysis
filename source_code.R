...
process_data <- function(sheet_name, data_file = data) {
  ...
  # Read sheet, with checks that file and sheets exist
  if (!file.exists(data_file)) stop(sprintf("File not found: %s", data_file))
  sheets <- tryCatch(readxl::excel_sheets(data_file), error = function(e) NULL)
  if (is.null(sheets) || !(sheet_name %in% sheets)) {
    stop(sprintf("Sheet '%s' not found in %s. Available: %s",
                 sheet_name, data_file, paste(sheets %||% character(0), collapse = ", ")))
  }
  
  df <- read_excel(data_file, sheet = sheet_name)  # Load Excel sheet into DataFrame
  
  # Check that "cluster" column is not missing
  if (!("cluster" %in% colnames(df))) stop(sprintf("Missing column 'cluster' in sheet '%s'", sheet_name))
  ...
  # Check that groups match the pattern
  if (length(sample_cols) == 0) stop(sprintf("No group/cluster columns matched pattern '%s'", pattern))
  ...
  lapply(groups, function(grp, df) {
    ...
    # Check that columns exist for each group
    if (ncol(col_range) == 0) {
      message(sprintf("[WARN] No columns found for group '%s' in sheet '%s' ??? returning empty matrix.", grp, sheet_name))
...
compute_correlation <- function(data) {
  ...
  if (!all(apply(m, 2, is.numeric))) stop("All columns must be numeric for correlation.")
  tryCatch(Hmisc::rcorr(m, type = "pearson"),
           error = function(e) stop(sprintf("Correlation failed: %s", e$message)))
  }
...
flatten_corr_matrix <- function(cormat, pmat, nmat) {
  ...
  # Checks: make sure the datasets are square numeric matrices with the same dimensions
  if (!is.matrix(cormat) || !is.matrix(pmat) || !is.matrix(nmat))
    stop("flatten_corr_matrix: cormat, pmat, and nmat must be matrices.")
  if (!is.numeric(cormat) || !is.numeric(pmat) || !is.numeric(nmat))
    stop("flatten_corr_matrix: cormat, pmat, and nmat must be numeric.")
  if (nrow(cormat) != ncol(cormat) || nrow(pmat) != ncol(pmat) || nrow(nmat) != ncol(nmat))
    stop("flatten_corr_matrix: cormat, pmat, and nmat must be square matrices.")
  if (!all(dim(cormat) == dim(pmat)) || !all(dim(cormat) == dim(nmat)))
    stop("flatten_corr_matrix: cormat, pmat, and nmat must have the same dimensions.")
...
filter_correlations <- function(cor_results, experiment_name) {
  # Make sure the needed columns are not missing
  if (is.null(cor_results$r) || is.null(cor_results$P) || is.null(cor_results$n))
    stop("filter_correlations: cor_results must contain r, P, and n.")
  df <- flatten_corr_matrix(cor_results$r, cor_results$P, cor_results$n)
  df <- df %>% mutate(Experiment = experiment_name)
  
  # If no correlations were found
  if (nrow(df) == 0) {
    warning(sprintf("filter_correlations: no correlations found for '%s'; returning empty results.", experiment_name))
    return(list(all = df, positive = df, negative = df))
  }
...
filter_pclrc <- function(cor_df, pclrc_df) {
  ...
  # Make sure the needed columns are not missing
  if (!all(c("Feature1","Feature2","Correlation") %in% colnames(cor_df)))
    stop("filter_pclrc: 'cor_df' must have columns: Feature1, Feature2, Correlation.")
  if (!all(c("Feature1","Feature2") %in% colnames(pclrc_df)))
    stop("filter_pclrc: 'pclrc_df' must have columns: Feature1, Feature2.")
  ...
  out <-bind_rows(common_direct, common_reverse)
  
  # Check if no overlapping was found
  if (nrow(out) == 0) {
    warning("filter_pclrc: no overlapping edges found, returning empty result.")
  }
...
perform_batch_correlation_analysis <- function(datasets, pclrc_edge_paths) {
  ...
  # Ensure overlap uses matching names
  if (!all(names(filtered_results) %in% names(pclrc_edges))) {
    warning("perform_batch_correlation_analysis: some groups in 'filtered_results' are missing in 'pclrc_edge_paths'. Using available groups only.")
  }
...
common_corrs <- function(df1, df2) {
  ...
  # Make sure the needes columns exist
  req <- c("Feature1","Feature2","Correlation")
  if (!all(req %in% colnames(df1))) stop("common_corrs: 'df1' must have columns: Feature1, Feature2, Correlation.")
  if (!all(req %in% colnames(df2))) stop("common_corrs: 'df2' must have columns: Feature1, Feature2, Correlation.")
  
  # Make sure the correlation column is numeric
  if (!is.numeric(df1$Correlation) || !is.numeric(df2$Correlation)) stop("common_corrs: 'Correlation' must be numeric in both data frames.")
  ...
  out <- bind_rows(common_1, common_2)
  # Check if there are no common correlations
  if (nrow(out) == 0) {
    warning("common_corrs: no shared correlations found; returning empty result.")
  }
...
create_igraph <- function(df) {
  ...
  # Ensure the datasets are data frames, that the required columns exist and that the correlation weights are numeric
  if (is.null(df) || !is.data.frame(df)) stop("create_igraph: 'df' must be a data.frame.")
  req <- c("Feature1", "Feature2", "Correlation")
  if (!all(req %in% colnames(df))) {
    stop(sprintf("create_igraph: missing required columns: %s",
                 paste(setdiff(req, colnames(df)), collapse = ", ")))
  }
  if (nrow(df) == 0) return(igraph::make_empty_graph())
  if (!is.numeric(df$Correlation)) stop("create_igraph: 'Correlation' must be numeric.")
  if (any(is.na(df$Feature1) | is.na(df$Feature2))) stop("create_igraph: Feature1/Feature2 contain NA.")
...
add_singltons <- function(igraph_common,igraph1,igraph2){
  ...
  # Make sure the variables are igraph objects
  if (is.null(igraph_common) || !inherits(igraph_common, "igraph")) stop("add_singltons: 'igraph_common' must be an igraph object.")
  if (is.null(igraph1) || !inherits(igraph1, "igraph")) stop("add_singltons: 'igraph1' must be an igraph object.")
  if (is.null(igraph2) || !inherits(igraph2, "igraph")) stop("add_singltons: 'igraph2' must be an igraph object.")
  
  # Fetch vertex names (may be NULL for unnamed graphs)
  n1 <- names(igraph::V(igraph1)); if (is.null(n1)) n1 <- character(0)
  n2 <- names(igraph::V(igraph2)); if (is.null(n2)) n2 <- character(0)
  nc <- names(igraph::V(igraph_common)); if (is.null(nc)) nc <- character(0)
...
for (bact in names(igraphs_all)) {
  ...
  cc_pos <- igraph::transitivity(ig_pos, type = "global", vids = V(ig_pos),
                         weights = 1/abs(E(ig_pos)$weight), isolates = "zero")
  cc_neg <- igraph::transitivity(ig_neg, type = "global", vids = V(ig_neg),
...
shuffle_fun <- function(igraph) {
  ...
  if (is.null(igraph) || !inherits(igraph, "igraph")) stop("shuffle_fun: input must be an igraph object.")
  
  g1 = igraph::rewire(igraph, with = keeping_degseq(niter = ecount(igraph) * 10))
  
  # Shuffle weights if present and sized correctly
  w <- igraph::E(igraph)$weight
  if (!is.null(w) && length(w) == igraph::ecount(g1)) {
    igraph::E(g1)$weight <- sample(w, replace = FALSE)
  } else if (is.null(w)) {
    warning("shuffle_fun: no weights on original graph, leaving weights unchanged.")
  } else {
    warning("shuffle_fun: weight length mismatch, leaving weights unchanged.")
  }  
  return(g1)
}
...
process_and_shuffle <- function(pos_igraph, neg_igraph, num_shuffles) {
  # Check that the input variables pos_igraph and neg_igraph are igraph objects
  if (is.null(pos_igraph) || !inherits(pos_igraph, "igraph"))
    stop("process_and_shuffle: 'positive_igraph' must be an igraph.")
  if (is.null(neg_igraph) || !inherits(neg_igraph, "igraph"))
    stop("process_and_shuffle: 'negative_igraph' must be an igraph.")
  combined_igraph <- process_network(pos_igraph, neg_igraph)
  if (!is.numeric(num_shuffles)) 
    stop("process_and_shuffle: 'num_shuffles' must be numeric.")
...
# Here 10 shuffles are performed per group for runtime reasons (in the analysis for the paper 1000 shuffles were used)
...
jaccard_rnd_vs_real_networks <- function(list_of_rnd, list_of_true){
  ...
  if (length(list_of_true) == 0 || length(list_of_rnd) == 0) stop("jaccard_rnd_vs_real_networks: input lists cannot be empty.")
...
modularity_using_infomap_multi <- function(multilayer_net) {
  ...
  # Make sure the input is a list, and that the needed fields exist
  if (is.null(multilayer_net) || !is.list(multilayer_net))
    stop("modularity_using_infomap_multi: 'multilayer_net' must be a list.")
  req <- c("extended_ids", "nodes", "layers")
  if (!all(req %in% names(multilayer_net)))
    stop(sprintf("modularity_using_infomap_multi: missing fields in 'multilayer_net': %s",
                 paste(setdiff(req, names(multilayer_net)), collapse = ", ")))
...
z_score_multi <- function(i, modules, stats) {
  ...
  # Ensure the input datasets are data frames and that the needed columns exist
  if (is.null(modules) || !is.data.frame(modules)) stop("z_score_multi: 'modules' must be a data.frame.")
  if (is.null(stats)    || !is.data.frame(stats))    stop("z_score_multi: 'stats' must be a data.frame.")
  req_mod <- c("state_node","module","k_m")
  req_sta <- c("module","k_m_avg","k_m_sd")
  if (!all(req_mod %in% names(modules))) stop("z_score_multi: 'modules' must contain columns: state_node, module, k_m.")
  if (!all(req_sta %in% names(stats)))   stop("z_score_multi: 'stats' must contain columns: module, k_m_avg, k_m_sd.")
...
c_score_multi <- function(i, modules, edges) {
  ...
  # Ensure the input datasets are data frames and that the needed columns exist
  if (is.null(modules) || !is.data.frame(modules)) stop("c_score_multi: 'modules' must be a data.frame.")
  if (is.null(edges)   || !is.data.frame(edges))   stop("c_score_multi: 'edges' must be a data.frame.")
  if (!all(c("state_node","k_total") %in% names(modules)))
    stop("c_score_multi: 'modules' must contain columns: state_node, k_total.")
  if (!all(c("sn_from","sn_to","m_from","m_to") %in% names(edges)))
    stop("c_score_multi: 'edges' must contain columns: sn_from, sn_to, m_from, m_to.")
...
roles_fun <- function(modules_list,multi_net){
  ...
  # Ensure the input modules_list is a data frame and that the needed columns exist
  if (is.null(modules_list) || !is.data.frame(modules_list)) stop("roles_fun: 'modules_list' must be a data.frame.")
  if (!all(c("node_id","layer_id","state_node","module") %in% names(modules_list)))
    stop("roles_fun: 'modules_list' must contain columns: node_id, layer_id, state_node, module.")
  
  # Ensure required columns exist in extended_ids
  need_ext <- c("node_from","node_to","layer_from")
  if (!all(need_ext %in% names(multi_net$extended_ids)))
    stop(sprintf("roles_fun: 'extended_ids' must contain: %s", paste(need_ext, collapse = ", ")))
  if (!("weight" %in% names(multi_net$extended_ids))) multi_net$extended_ids$weight <- 1
  ...
  # Make sure intra-layer edges exist
  if (!nrow(expanded_intra)) {
    stop("roles_fun: no intra-layer edges after joining; cannot compute roles.")
  }
  ...
  # Make sure within-module edges exist 
  if (!nrow(k_m)) stop("roles_fun: no within-module edges, cannot compute k_m.")
...
create_NMI_matrix <- function(network_par_1,network_par_2){
  ...
  # Ensure the input datasets are data frames and that the needed columns exist
  if (is.null(network_par_1) || !is.data.frame(network_par_1)) stop("create_NMI_matrix: 'network_par_1' must be a data.frame.")
  if (is.null(network_par_2) || !is.data.frame(network_par_2)) stop("create_NMI_matrix: 'network_par_2' must be a data.frame.")
  req <- c("node_id","layer_id","module")
  if (!all(req %in% names(network_par_1))) stop("create_NMI_matrix: 'network_par_1' must contain node_id, layer_id, module.")
  if (!all(req %in% names(network_par_2))) stop("create_NMI_matrix: 'network_par_2' must contain node_id, layer_id, module.")
...
NMI_networks <- function(list_df){
  ...
  # Ensure the input list includes dataframes and that the needed columns exist
  
  req <- c("node_id","layer_id","module")
  for (k in seq_along(list_df)) {
    if (!is.data.frame(list_df[[k]])) stop(sprintf("NMI_networks: element %d is not a data.frame.", k))
    if (!all(req %in% names(list_df[[k]])))
      stop(sprintf("NMI_networks: element %d must contain columns: %s", k, paste(req, collapse = ", ")))
  }
...
node_info <- function(igraph){
  # Ensure the input is an igraph object
  if (is.null(igraph) || !inherits(igraph, "igraph")) stop("node_info: input must be an igraph object.")
  
  # Ensure weights exist and avoid division by zero
  w <- igraph::E(igraph)$weight
  if (is.null(w)) {
    warning("node_info: no edge weights, using weight = 1 for all edges.")
    w <- rep(1, ne)
  }
  w_abs <- abs(w)
  w_abs[is.na(w_abs) | w_abs == 0] <- .Machine$double.eps  # prevent Inf in 1/abs(weight)
  
  CC <- tryCatch(
    igraph::transitivity(igraph, type = "local", vids = igraph::V(igraph), weights = 1 / w_abs, isolates = "zero"),
    error = function(e) rep(NA_real_, nv)
  )
  bet <- tryCatch(
    igraph::betweenness(igraph, v = igraph::V(igraph), directed = FALSE, weights = 1 / w_abs, normalized = TRUE),
    error = function(e) rep(NA_real_, nv)
  )
  degree_vec <- tryCatch(
    igraph::degree(igraph, v = igraph::V(igraph), mode = "all", loops = FALSE, normalized = FALSE),
    error = function(e) rep(NA_real_, nv)
  )
  nei <- tryCatch(
    influential::neighborhood.connectivity(igraph, vertices = igraph::V(igraph), mode = "all", verbose = FALSE),
    error = function(e) rep(NA_real_, nv)
  )
...
extract_and_merge_metric <- function(igraphs_list, metric) {
  ...
  # Make sure the elements in the igraph list are named
  if (is.null(names(igraphs_list)) || any(names(igraphs_list) == ""))
    stop("extract_and_merge_metric: all elements of 'igraphs_list' must be named.")
